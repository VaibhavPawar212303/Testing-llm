...
class BaseReportingModel(DeepEvalBaseLLM):
    ...
    def save_report(self, report_dir: str, prefix: str = "llm_analysis"):
        ...
        try:
            ...
            print(f"\n{Colors.OKGREEN}📄 Report saved: {report_file}{Colors.ENDC}")
            return report_file
            
        except Exception as e:
            print(f"{Colors.WARNING}⚠️  Report save failed: {e}{Colors.ENDC}")
            return None
    ...
    def _save_markdown_report(self, filepath: str, data: Dict[str, Any]):
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            if data['generation_time']['duration_seconds']:
                f.write(f"## ⏱️ Generation Time: {data['generation_time']['duration_seconds']:.2f}s\n\n")
            if data['metrics']:
...
class OllamaModel(BaseReportingModel):
    ...
    def _display_metrics(self):
        ...
        if 'duration_ms' in self.last_metrics: print(f"⏱️  Duration: {self.last_metrics['duration_ms']/1000:.2f}s")
